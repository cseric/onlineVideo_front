<template>
  <div>
    <el-empty v-if="followList.length === 0" description="暂无关注"></el-empty>
    <div v-else>
...
export default {
  name: 'userFollow',
  async created () {
    await this.getFollowList(this.$route.params.uid)
  },
  data () {
    return {
      followList: [],
  ...
  methods: {
    async getFollowList (uid) {
      const { data: res } = await getFollowListAPI({ uid: uid, ...this.params })
      if (res.code === 2000) {
        this.followList = res.data.followUsers
        this.total = res.data.total
      }
    },
    handleCurrentChange (newPage) {
      this.params.page = newPage
      this.getFollowList(this.$route.params.uid)
    },
    toUserSpace (uid) {
